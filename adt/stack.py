...
def push(input_stack, input_item, stack_size, n_blocks = 3, block_size = 2):
  push_net = {}
  push_net['combine'] = prev_layer = ConcatLayer([input_stack, input_item])
  layer_width = stack_size
  print("pop layer_width", layer_width)
  wx = batch_norm(DenseLayer(prev_layer, layer_width, nonlinearity = lasagne.nonlinearities.rectify, W=lasagne.init.HeNormal(gain='relu')))
...
# Push(Stack, Item) -> Stack
def pop(input_stack, stack_size, item_size, n_blocks = 3, block_size = 2):
  pop_net = {}
  prev_layer = x = input_stack
  layer_width = stack_size+item_size
  print("layer_width", layer_width)
  wx = batch_norm(DenseLayer(x, layer_width, nonlinearity = lasagne.nonlinearities.rectify, W=lasagne.init.HeNormal(gain='relu')))
...
def main(X_train, stack_size = 100, nbatch = 256, item_size = 28*28):
